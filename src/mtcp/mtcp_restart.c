...
//  rinfo.text_offset = -1;
  ...
  while (argc > 0) {
    ...
    if (mtcp_strcmp(argv[0], "--use-gdb") == 0) {
      ...
      shift; shift;
    } else if (mtcp_strcmp(argv[0], "--cuda") == 0) {
      rinfo.restore_split_process = 1;
      shift;
...
# warning __FUNCTION__ "TODO: Implementation for ARM64"
  ...
  if (rinfo_ptr->restore_split_process) {
    /*
      Idea:
      1. mtcp needs to remember own region and unmap only those regions later
      2. mmap upper-half's region to the process address space once
      3. setup the lower-half and run __libc_csu_init in order to register
         cuda components.
      4. mmap the upper-half again to avoid any side-effect by previous step.
      5. finally return to the checkpointed process.

      1 and 2 are already done.
    */
//    int rc = setupLowerHalfInfo();
    // if (rc < 0) {
    //   DPRINTF("Failed to set up lhinfo for the upper half. Exiting...\n");
    //   exit(-1);
    // }
    /* Restore memory areas */
    DPRINTF("restoring memory areas second time.\n");
    // readmemoryareas(restore_info.fd);
      /* Restore libc again as the */
    DPRINTF("Memory is now restored.  Will next restore libc internals again.\n");
    restore_libc(&restore_info.motherofall_tls_info, restore_info.tls_pid_offset,
                restore_info.tls_tid_offset, restore_info.myinfo_gs);
    /*
      Replaying the cuda memory related calls should be handled by the plugin
      before entering to the program.
    */
  }
